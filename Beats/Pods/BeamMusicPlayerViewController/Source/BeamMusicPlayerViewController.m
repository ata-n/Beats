...
@property (nonatomic,weak) IBOutlet GLView* waveView;
...
- (void)viewDidLoad
{
    ...
    // Connect waveform
    if ([self.delegate respondsToSelector:@selector(musicPlayer:needsWaveFormIn:)]) {
        [self.delegate musicPlayer:self needsWaveFormIn:self.waveView];
    }
...
//        [[self.albumArtReflection layer] addAnimation:transition forKey:@"SlideOutandInImagek"];
    ...
    // WaveForm
    /*
    if ([self.delegate respondsToSelector:@selector(musicPlayer:needsWaveFormIn:forTrack:)]) {
        [self.delegate musicPlayer:self needsWaveFormIn:self.waveView forTrack:self.currentTrack];
    }
     */
}
...
-(void)play {
    ...
    if ([self.delegate respondsToSelector:@selector(musicPlayerShouldStartPlaying:)]) {
        shouldPlay = [self.delegate musicPlayerShouldStartPlaying:self];
    }
    
    if ( !self.playing && shouldPlay){
        self->playing = YES;
...
-(void)pause {
    ...
    if ([self.delegate respondsToSelector:@selector(musicPlayerShouldStopPlaying:)]) {
        shouldPause = [self.delegate musicPlayerShouldStopPlaying:self];
    }
    
    if ( self.playing  && shouldPause){
        self->playing = NO;
...
-(void)updateSeekUI {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        /*
        UIView *waveView = (UIView *)self.waveView;
        CGRect newFrame = waveView.frame;
        newFrame.origin.x = -self.currentPlaybackPosition * waveView.frame.size.width / self.currentTrackLength;
        [UIView animateWithDuration:1 delay:0 options:UIViewAnimationOptionCurveLinear animations:^{
            waveView.frame = newFrame;
        } completion:^(BOOL finished) {
            
        }];
         */
        //NSLog(@"pos: %f, width: %f, length: %f", self.currentPlaybackPosition, waveView.frame.size.width, self.currentTrackLength);
    });
}
